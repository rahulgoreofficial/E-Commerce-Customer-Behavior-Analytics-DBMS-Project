...
def seed_sessions_and_events(cur, customer_ids, product_ids):
    ...
    # Pre-fetch real review samples
    try:
        real_reviews = review_loader.sample_reviews_for_seeding(1200)
        review_pool = {1: [], 2: [], 3: [], 4: [], 5: []}
        for rev in real_reviews:
            r = rev['rating']
            if r in review_pool:
                review_pool[r].append(rev)
    except Exception as e:
        print(f"  Warning: could not load real review dataset: {e}")
        review_pool = {}
    ...
    for _ in range(NUM_SESSIONS):
        ...
        if random.random() < 0.35:
            ...
            if will_purchase:
                ...
                # ── Review (35% chance after purchase) ──
                if random.random() < 0.35:
                    review_date = event_time + timedelta(days=random.randint(1, 30))
                    review_prod = random.choice(cart_products)
                    rating = random.choices([1, 2, 3, 4, 5], weights=[8, 10, 12, 32, 38])[0]
                    
                    # Extract from real review pool if available
                    if review_pool and review_pool.get(rating):
                        sampled_item = random.choice(review_pool[rating])
                        text = (sampled_item['review_title'] + ": " + sampled_item['review_text'])[:500].strip()
                        sentiment = sampled_item['sentiment_score']
                    else:
                        review_texts = {
                            5: "Excellent product! Exceeded expectations and delivered promptly.",
                            4: "Good product and great value for money. Very satisfied.",
                            3: "Average quality, works as expected but packaging could improve.",
                            2: "Disappointing experience, delivery was delayed and product looks cheap.",
                            1: "Terrible quality and rude customer service. Avoid buying this.",
                        }
                        text = review_texts.get(rating, "OK")
...
def seed_additional_real_reviews(cur, customer_ids, product_ids, target_count=600):
    """Seed a rich batch of real Amazon reviews across customer catalog."""
    print(f"  Ingesting {target_count} authentic Amazon reviews with NLP sentiment scores...")
    try:
        real_reviews = review_loader.sample_reviews_for_seeding(target_count)
        inserted = 0
        for rev in real_reviews:
            cust_id = random.choice(customer_ids)
            prod_id = random.choice(product_ids)
            rating = rev['rating']
            text = (rev['review_title'] + ": " + rev['review_text'])[:600].strip()
            sentiment = rev['sentiment_score']
            rev_date = random_date(120, 1)

            cur.execute("""
                INSERT INTO review (customer_id, product_id, rating, review_text, 
                                    sentiment_score, review_date, is_verified)
                VALUES (%s, %s, %s, %s, %s, %s, TRUE)
            """, (cust_id, prod_id, rating, text, sentiment, rev_date))
            inserted += 1

        print(f"  Successfully inserted {inserted} verified Amazon customer reviews into database.")
    except Exception as e:
        print(f"  Warning: could not seed additional real reviews: {e}")
...
def run_seed():
    ...
    with db.transaction() as cur:
        ...
        seed_additional_real_reviews(cur, customer_ids, product_ids, target_count=600)
        seed_campaigns(cur, customer_ids)

        # Update product average ratings and total reviews
        print("  Recalculating product ratings from customer reviews...")
        cur.execute("""
            UPDATE product p
            SET avg_rating = COALESCE(sub.avg_r, 0),
                total_reviews = COALESCE(sub.cnt, 0)
            FROM (
                SELECT product_id, ROUND(AVG(rating)::NUMERIC, 2) AS avg_r, COUNT(*) AS cnt
                FROM review
                GROUP BY product_id
            ) sub
    ...
    print("=" * 60)
    print("Database seed completed with Real Review Intelligence!")
    print("=" * 60)
