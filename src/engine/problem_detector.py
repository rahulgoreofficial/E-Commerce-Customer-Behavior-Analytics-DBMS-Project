...
class ProblemDetector:
    ...
    def detect_all_problems(self) -> List[DetectedProblem]:
        ...
        problems.extend(self._detect_review_complaint_spikes())
    ...
    def _detect_review_complaint_spikes(self) -> List[DetectedProblem]:
        """Detect spikes in negative customer review themes (delivery, defective items, support)."""
        problems = []
        try:
            # Query recent negative reviews (last 60 days)
            neg_reviews = db.execute_query("""
            ...
            if not neg_reviews or len(neg_reviews) < 5:
                return problems

            delivery_kw = ["delivery", "driver", "courier", "late", "package", "lost", "door"]
            quality_kw = ["broken", "defective", "cheap", "damaged", "poor quality", "waste"]
            support_kw = ["customer service", "refund", "return", "support", "rude", "agent", "refused"]
            ...
            for r in neg_reviews:
                ...
                is_del = any(w in text for w in delivery_kw)
                is_qual = any(w in text for w in quality_kw)
                is_sup = any(w in text for w in support_kw)

                if is_del: delivery_hits += 1
                if is_qual: quality_hits += 1
                if is_sup: support_hits += 1

                if len(sample_quotes) < 2 and (is_del or is_qual or is_sup):
                    sample_quotes.append(f'"{r["review_text"][:90]}..."')

            total_neg = len(neg_reviews)

            # Flag delivery failure spike
            if delivery_hits >= 6:
                p = DetectedProblem(
                    problem_id=f"DELIVERY_SPIKE_{datetime.now().strftime('%Y%m%d')}",
                    problem_type="logistics_complaint_spike",
                    title="Logistics & Delivery Complaint Spike Detected",
                    description=f"{delivery_hits} recent negative reviews mention delivery delays, missing drivers, or damaged packages.",
                    severity='high' if delivery_hits > 15 else 'medium',
                    evidence=[
                        Evidence(f"{delivery_hits} of {total_neg} negative reviews flag delivery friction",
                                 "delivery_complaints", delivery_hits, 0, 0, "increased"),
                        Evidence(f"Sample voice of customer: {' | '.join(sample_quotes[:1])}",
                                 "customer_sentiment", float(total_neg), 0, 0, "increased")
                    ],
                    metrics={"delivery_complaints": delivery_hits, "total_neg_reviews": total_neg},
                    confidence=0.85,
                    recommendations=[
                        Recommendation("Audit 3PL courier performance and on-time delivery SLAs",
                                       "Frequent delivery complaints damage retention and brand loyalty",
                                       "high", "investigate"),
                        Recommendation("Implement automated real-time SMS tracking updates",
                                       "Proactive communication reduces delivery anxiety and calls",
                                       "medium", "implement")
                    ]
                )
                problems.append(p)

            # Flag customer support / refund friction
            if support_hits >= 6:
                p = DetectedProblem(
                    problem_id=f"SUPPORT_FRICTION_{datetime.now().strftime('%Y%m%d')}",
                    problem_type="support_friction_spike",
                    title="Customer Service & Refund Dispute Bottleneck",
                    description=f"{support_hits} negative reviews report unresolved returns or unresponsive support agents.",
                    severity='medium',
                    evidence=[
                        Evidence(f"{support_hits} reviews highlight refund disputes and support delays",
                                 "support_complaints", support_hits, 0, 0, "increased")
                    ],
                    metrics={"support_complaints": support_hits, "total_neg_reviews": total_neg},
                    confidence=0.80,
                    recommendations=[
                        Recommendation("Streamline 1-click refund approval for verified returns",
                                       "Refund friction is a primary driver of customer churn",
                                       "high", "implement"),
                        Recommendation("Implement 24/7 AI chatbot assistance for order lookups",
                                       "Reduces support response times by up to 60%",
                                       "medium", "test")
                    ]
                )
                problems.append(p)
            ...
            print(f"Warning: could not detect review complaint spikes: {e}")
